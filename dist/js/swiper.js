...
/******/ 	return __webpack_require__(__webpack_require__.s = "./app/js/plugins.js");
/******/ })
/************************************************************************/
/******/ ({

/***/ "./app/js/flurry/jquery.flurry.js":
/*!****************************************!*\
  !*** ./app/js/flurry/jquery.flurry.js ***!
  \****************************************/
...
(function ($, undefined) {
  ...
  function createFlake(options, $container, containerWidth) {
    ...
    var character = options.character.length === 1 ? options.character : options.character.charAt(Math.round(randomNumberInRange(0, options.character.length - 1))); // Set the flake's starting position to a random number between the container width, including additional space for the wind setting

    var startX = randomNumberInRange(-Math.abs(options.wind), containerWidth + Math.abs(options.wind)); // Set the flake's ending X translation to a random number based on the wind and windVariance options

    var endX = startX + randomNumberInRange(options.wind - options.windVariance, options.wind + options.windVariance); // Set the flake's font size to a random number between the small and large options

    var fontSize = randomNumberInRange(options.small, options.large); // Set the flake's speed to a random number based on the speed setting and the randomized fontSize

    var speed = options.speed / ((randomNumberInRange(fontSize * 1.2, fontSize * 0.8) - options.small) / (options.large - options.small) + 0.5); // Set the flake's ending Y translation based on the height setting and the randomized fontSize

    var endY = options.height - fontSize; // Set the flake's rotation to a random degree based on the rotation and rotationVariance options

    var endRotation = randomNumberInRange(options.rotation - options.rotationVariance, options.rotation + options.rotationVariance); // Set the flake's color based on color options

    var color = Array.isArray(options.color) ? options.color[Math.floor(Math.random() * options.color.length)] : options.color; // Create object to store final CSS properties for the flake
  ...
  function Plugin(element, options) {
    ...
    var self = this;
    var el = element;
    var $el = $(element); // Extend default options with those supplied by user.

    options = $.extend({
      height: $el.height() > 200 ? 200 : $el.height(),
      // default to 200px or the height of the element, whichever is smaller
      useRelative: $el.is('body') ? false : true // default to false for the body element and true for all other elements

    }, $.fn[pluginName].defaults, options); // Ensure options that should be numbers are numbers
    ...
    function init() {
      ...
      var $container = $(document.createElement('div')).addClass('flurry-container').css({
        ...
        'pointer-events': 'none'
      }).prependTo($el); // On window resize, recalculate the width used to generate flakes within
      ...
      }, options.frequency); // Call onInit hook
    ...
    function destroy() {
      ...
      $el.each(function () {
        var el = this;
        var $el = $(this); // Add code to restore the element to its original state...
        // Cancel snowflake generation

        clearRequestInterval(self.flakeInterval); // Remove container

        $el.find('.flurry-container').remove(); // Call onDestroy hook

        hook('onDestroy'); // Remove Plugin instance from the element.
    ...
    init(); // Expose methods of Plugin we wish to be public.
...
/* harmony import */ var _flurry_jquery_flurry_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./flurry/jquery.flurry.js */ "./app/js/flurry/jquery.flurry.js");
/* harmony import */ var _flurry_jquery_flurry_js__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_flurry_jquery_flurry_js__WEBPACK_IMPORTED_MODULE_1__);
// Swiper 4.5.0

window.Swiper = swiper__WEBPACK_IMPORTED_MODULE_0__["default"]; // flurry 1.1.0


window.flurry = _flurry_jquery_flurry_js__WEBPACK_IMPORTED_MODULE_1___default.a;
